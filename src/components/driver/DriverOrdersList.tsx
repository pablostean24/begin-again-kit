...
const DriverOrdersList = ({ driverId }: DriverOrdersListProps) => {
  ...
  const OrderCard = ({ order, isAvailable = false }: { order: DriverOrder; isAvailable?: boolean }) => (
    <Card key={order.id} className={`border-l-4 ${isAvailable ? 'border-l-green-500' : 'border-l-blue-500'}`}>
      <CardHeader>
        ...
        <div className="flex gap-2 pt-4">
          {isAvailable ? (
            <Button
              ...
              disabled={assignOrderToDriver.isPending}
            >
              <Plus className="w-4 h-4 mr-1" />
              {assignOrderToDriver.isPending ? 'Asignando...' : 'Tomar Pedido'}
            </Button>
          ) : (
            <>
      ...
      <Tabs defaultValue="assigned" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="assigned" className="flex items-center">
            ...
            Mis Asignados ({assignedOrders?.length || 0})
          </TabsTrigger>
        ...
        <TabsContent value="available" className="space-y-4">
          {availableOrders?.length === 0 ? (
            <Card>
              <CardContent className="py-8 text-center text-gray-500">
                No hay pedidos listos disponibles para entrega
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-4">
